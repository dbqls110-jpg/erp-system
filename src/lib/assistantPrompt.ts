...
const UPDATE_FORMAT = [
  ...
  "  사람이 카드의 반영 버튼을 눌렀을 때만 Google Sheets '호스트 등록 공간'과 '공간DB'에 동시에 반영됩니다. 별도의 '공간 등록 접수' 탭에 먼저 저장하지 않습니다.",
  "  사용자가 '공간등록 1번', '공간등록 30번'처럼 번호를 지정하면 fields.spaceNumber에 그 번호를 숫자로 넣으세요. 번호 1은 호스트 시트의 데이터 첫 행인 물리 2행, 번호 30은 물리 31행을 뜻합니다. 번호가 없으면 spaceNumber를 생략합니다.",
  "  spaceName은 필수입니다. 알고 있는 값만 fields에 넣고, 모르는 연락처·주소·요금은 지어내지 마세요.",
  "  area·capacity·dailyRate·weekdayRate·weekendHolidayRate·minimumRentalDays·parkingCount·areaPyeong·rentableTotalArea·roomCount·parkingSpaces·deposit·managementFee 같은 숫자 칸을 모르면 칸을 생략하거나 null로 두세요. '확인 필요'·'협의'·'-'를 숫자 칸에 넣지 마세요.",
  "  단일 요금이 아니라 평일·주말 요금이 따로 있으면 dailyRate 하나로 임의 통합하지 말고 weekdayRate와 weekendHolidayRate를 각각 만원 단위 숫자로 넣으세요. 최소 대관일은 minimumRentalDays, 부가세 포함 여부는 vatIncluded에 넣으세요.",
  "  사용할 수 있는 fields: spaceNumber, spaceName, contactName, relationship, phone, email, spaceType, address, desiredRegion, description, area, capacity, dailyRate, weekdayRate, weekendHolidayRate, minimumRentalDays, vatIncluded, negotiable, conditions, privacyConsentAt, photoPermission, cooling, restroom, wifi, parkingCount, fireNotAllowed, drillingNotAllowed, noiseLimit, equipmentRental, nightWork, foodAllowed, extraConditions, areaPyeong, rentableFloors, rentableTotalArea, rentableFloorArea, outdoorYard, kitchen, usage, storageOffice, roomCount, powerCapacity, elevator, freightElevator, ooh, wasteDisposal, drilling, accessHours, parkingAvailable, parkingSpaces, floorPlan, ceilingHeight, lighting, wiredInternet, floorFinish, deposit, managementFee, tourMethod.",
  "  질문에 사진을 첨부했다면 사진은 서버가 지정된 공간 번호와 이름으로 '천우영 프로젝트/공간 등록/V0001 공간명' 폴더에 옮겨 호스트 등록 공간 행에 연결하므로 photoFolderUrl·photoCount를 지어내지 마세요.",
  "",
  "```erp-update",
  '{"target":"space_registration_create","label":"호스트 등록 공간 반영","fields":{"spaceNumber":1,"spaceName":"공간1","address":"서울시 ...","spaceType":"스튜디오","capacity":100,"weekdayRate":88,"weekendHolidayRate":110,"minimumRentalDays":3,"vatIncluded":"별도"},"reason":"지정한 호스트 등록 공간 번호와 공간DB에 반영"}',
  "```",
  "",
