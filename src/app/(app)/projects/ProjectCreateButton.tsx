          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                type="file"
                name="quoteFile"
                accept=".pdf,.txt,.csv,.json,.doc,.docx,image/*"
                onChange={handleQuoteFileChange}
                disabled={loading || analyzing}
                className="cursor-pointer"
              />
              <p className="text-xs text-muted-foreground">PDF(텍스트형), CSV, TXT는 금액을 자동 추출합니다. 스캔 이미지·워드 파일은 원본만 보관하고 금액은 직접 확인해 주세요.</p>
              {analyzing && (
