...
export default async function FinancePage() {
  ...
  const [budget, expenses] = await Promise.all([
    ...
    prisma.expense.findMany({
      where: { date: { gte: `${year}-${monthStr}-01`, lte: `${year}-${monthStr}-31` } },
      orderBy: { date: "desc" },
    }),
  ]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="border-ash-gray shadow-[var(--shadow-sm)]">
          ...
          <CardContent>
            <p className="text-2xl font-bold text-warm-fade">{totalExpense.toLocaleString()}원</p>
              ...
              {expenses.map((e) => (
                <div key={e.id} className="flex items-center justify-between py-2 border-b border-ash-gray last:border-0 text-sm">
                  <div className="flex items-center gap-3">
                    ...
                    {e.memo && <span className="text-smoke-gray text-xs">{e.memo}</span>}
                  </div>
