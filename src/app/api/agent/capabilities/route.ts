            ...
            requests: `FormatRequest[] (필수, 최대 50개) — 각 요소:
  {
    ...
    },
    {
      ...
      pythonBridge: {
        ...
        files: [
          "client.py — 메인 SSE 수신 루프 (운영시간·지수백오프·하트비트)",
          "protocol.py — AgentJob 데이터 클래스",
          "hermes_cli.py — Hermes CLI(hermes chat) 실행 공통 모듈",
          "preflight.py — 브릿지 시작 전 Hermes CLI 사전 점검",
          "runners/hermes.py — hermes_cli 통해 Hermes chat CLI 호출 (Anthropic API 직접 호출 없음)",
          "runners/marketer.py — hermes_cli 통해 Hermes chat CLI 호출 (Anthropic API 직접 호출 없음)",
          "start_hermes.cmd / start_hermes.ps1 — Windows 시작 스크립트",
          "start_marketer.cmd / start_marketer.ps1 — Windows 시작 스크립트",
          "check_hermes.cmd / check_marketer.cmd — Hermes CLI 사전 점검 스크립트",
          "hermes.env.example — 회사 PC(Hermes) 환경변수 예시",
          "marketer.env.example — 노트북(Marketer) 환경변수 예시",
        ],
        env: {
          ERP_BASE_URL: "https://erp-system-lojo.onrender.com",
          ERP_AGENT_API_KEY: "Hermes/Marketer 전용 브릿지 키 (Render 환경변수와 동일)",
          AGENT_TYPE: "hermes | marketer",
          AGENT_OPEN_HOUR: "8 (KST, 기본값)",
          AGENT_CLOSE_HOUR: "1 (익일 KST, 기본값)",
          HERMES_EXECUTABLE: "hermes.exe 전체 경로 (선택, 미지정 시 PATH·알려진 설치 위치에서 자동 탐색)",
        },
        note: "Anthropic API 키 불필요 — 각 PC에 로그인된 Hermes 구독 세션을 그대로 사용",
        backoff: "연결 오류 시 5→15→30→60→120→300초 지수 백오프",
        operatingHours: "08:00~익일 01:00 KST (환경변수로 조정 가능). 외 시간은 대기",
        install: "cd agent_bridge && pip install -r requirements.txt && cp hermes.env.example hermes.env (또는 marketer.env.example → marketer.env) 후 값 편집",
        start: {
          hermes: "agent_bridge\\start_hermes.cmd (또는 .ps1) — 사전 check_hermes.cmd 권장",
          marketer: "agent_bridge\\start_marketer.cmd (또는 .ps1) — 사전 check_marketer.cmd 권장",
        },
      },
    },
    {
