...
const outsiders: Record<string, Viewer> = {
  ...
  "공간에 연결된 공간 호스트": { id: "h1", role: "host", partnerId: null, customerId: null, venueId: "venue-a" },
  "연결 안 된 공간 호스트 레벨": { id: "h2", role: "host", partnerId: null, customerId: null },
  "레벨은 사원인데 공간에 연결": { id: "h3", role: "member", partnerId: null, customerId: null, venueId: "venue-b" },
};
...
describe("외부인에게 붙는 자료 조건", () => {
  ...
  it("공간 호스트는 프로젝트와 연결 고리가 없어 프로젝트·일정이 하나도 안 걸린다", () => {
    for (const key of ["공간에 연결된 공간 호스트", "연결 안 된 공간 호스트 레벨"]) {
      expect(projectWhereFor(outsiders[key])).toEqual({ id: { in: [] } });
      expect(calendarWhereFor(outsiders[key])).toEqual({ id: { in: [] } });
    }
  });
...
describe("파트너·공간 호스트 레벨의 메뉴 권한(기본 규칙)", () => {
  const rows: AccessRow[] = Object.entries(DEFAULT_MENU_RULES).flatMap(([menuKey, rule]) =>
    ["admin", "manager", "member", "partner", "host"].map((levelKey) => ({
      menuKey,
      levelKey,
  ...
  for (const level of ["partner", "host"]) {
    for (const menu of MENU_KEYS) {
      const shouldView = PARTNER_ALLOWED.has(menu.key);
      it(`${menu.label}: ${level} ${shouldView ? "보기만" : "차단"}`, () => {
        const access = resolveMenuAccess(level, menu.key, rows);
