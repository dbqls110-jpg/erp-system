...
python -c "import sys; sys.path.insert(0, sys.argv[1]); import preflight, logging; logging.basicConfig(stream=sys.stderr,level=logging.INFO,format='%%(message)s'); ok=preflight.run('marketer'); sys.exit(0 if ok else 1)" "%~dp0\"
if errorlevel 1 (
...
:: ── 중복 실행 방지 (lock의 PID가 실제 marketer client.py인지 명령줄로 검증) ──────
...
set "PIDFILE=%~dp0marketer.pid.tmp"
if exist "%PIDFILE%" del "%PIDFILE%"
powershell -NoProfile -NonInteractive -Command "try { $p = Start-Process -FilePath 'pythonw' -ArgumentList 'client.py' -WindowStyle Hidden -RedirectStandardOutput 'marketer.log' -RedirectStandardError 'marketer_error.log' -PassThru -ErrorAction Stop; Set-Content -Path 'marketer.pid.tmp' -Value $p.Id -NoNewline } catch { }"
...
echo [ERROR] Marketer 브릿지 프로세스 시작 실패.
exit /b 1
